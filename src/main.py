'''
Reference implementation of scaled NAPE algorithm.
...
def get_negative_samples(G, sorted_nodes=[]):
...
def get_neg_samples_deg_dist(G,nodes=[]):
	return random.sample(nodes,args.k_deg)


def get_necessary_objects(args, nx_G, nodes_with_degX):
...
def main(args):
	...
	neg_samples, degree_vec = get_negative_samples(nx_G,nodes)
	deg_neg_samples = get_neg_samples_deg_dist(nx_G,nodes)
	...
	pos_neigh, deg_pos_neigh = get_necessary_objects(args, nx_G, nodes_with_degX)
